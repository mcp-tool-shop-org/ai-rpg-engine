...
*   Environnement d'exécution principal : état du monde, événements, actions, cycles de jeu, relecture — stable depuis la version 1.0 ; relecture déterministe avec des octets identiques (compteur d’ID par instance, générateur de nombres aléatoires initialisé).
...
*   Capacités : coûts, délais de récupération, vérifications de statistiques, effets typés, vocabulaire de statut à 11 balises, sélection tenant compte de l’IA.
*   **Combat de groupe (v2.4) :** ciblage des alliés (soin/amélioration/résurrection), filtrage des zones d’effet sur les amis/ennemis, sélecteurs de cibles — un soigneur peut soigner un coéquipier ; la zone d’effet ennemie épargne les alliés.
*   **Effets de statut (v2.4) :** les modificateurs de statistiques passifs affectent le combat, effets DoT/HoT déterministes basés sur le compteur de cycles, déclencheurs réactifs à profondeur limitée (épines/réflexion).
...
*   **Boucle de jeu jouable (`run`) (v2.6) :** le jeu final est réel, pas une démo — les ennemis agissent en fonction de leurs profils d’intention d’IA (« agressif »/« prudent »/« territorial »/« calculateur »), un combat se termine par une victoire ou une défaite, vous pouvez enregistrer et reprendre, et les capacités et l’expérience sont disponibles dans le menu d’action. `run <path>` charge un jeu que vous avez préparé. Interface utilisateur finale composée avec un HUD clair et des couleurs accessibles (respecte `NO_COLOR` / non-TTY).
*   **L’atelier de conception de l’IA est fourni sous forme de sa propre commande « ai » (v2.6) :** `npm install -g @ai-rpg-engine/ollama` → `ai chat` — prépare, critique et équilibre le contenu par rapport à un modèle Ollama local.
...
*   API de configuration de la cognition (`cognition: CognitionCoreConfig | false`) pour le réglage de l’IA par monde initial.
*   Taxonomie des balises et utilitaires de validation pour la création de contenu.
*   **Le monde réagit (v2.7) :** les éliminations accumulent de la chaleur et érodent la sécurité du district ; un cycle de jeu mondial par tour génère des pressions cachées qui se manifestent sous forme de rumeurs (« Des murmures parviennent à vos oreilles… »), s’intensifient et expirent avec des conséquences ; les ~30 compositions d’événements créées se déclenchent lors de l’entrée dans la zone dans les 10 mondes initiaux — déterministe par graine, les districts plus dangereux en génèrent davantage, les éléments clés du boss sont protégés.
*   **Une raison de revenir (v2.7) :** une boucle de quête minimale sur le schéma déjà déployé depuis longtemps — les quêtes offrent des déclencheurs, suivent les objectifs d’élimination/d’atteinte/de progression et versent l’expérience et les objets exactement une fois ; quatre quêtes créées, un écran « Journal », des étapes de quête dans la narration du cycle.
*   **L’équipement affecte le combat (v2.7) :** `equip`/`unequip` modifie les valeurs réelles à travers la couche de statut que les formules de combat lisent déjà — aucun changement dans le code de combat ; le trident et le filet du gladiateur sont connectés de bout en bout avec un delta de chance de coup testé.
...
*   **`buildWorldStack()` (v2.7) :** l’ossature de composition stratégique à côté de `buildCombatStack()` — un seul appel assemble l’environnement, les factions, les rumeurs, les districts, les conséquences de la défaite, les événements et les quêtes ; plus l’écran « Ledger du directeur » pour la stratégie, un inspecteur de simulation avec `AI_RPG_DEBUG=1`, `inspect-save` protégé par les mêmes autorités que Continuer, et une jonction de migration des modules sur le chemin de restauration déployé.
*   **Agir sur l’économie vivante (v2.8) :** `createEconomyCore` initialise une économie par district lors du chargement du pack et la fait évoluer à chaque tour ; un nouveau verbe « vendre » fixe les prix des butins via `computeItemValue` (rareté / faction / provenance / contrebande) et modifie l’offre locale. Un seul fil a permis de mettre en œuvre cinq systèmes qui étaient inactifs dans la version 2.7 — le « MARKET OVERVIEW + FACTIONS » du directeur, l’arc et le déclencheur d’effondrement du marchand-prince à la fin du jeu, et quatre types de pression économique. **Vente uniquement ce cycle** (achat → v2.9).
*   **Compagnons (v2.8) :** un verbe « recruter » crée un groupe — état, balises et faction, afin qu’un compagnon se batte *avec* vous ; le combat des compagnons s’appuie sur la mécanique d’interception du cœur de combat (inactif jusqu’à ce que `isAlly` soit défini), les compagnons réagissent avec leur moral et peuvent partir, et le recrutement active sept consommateurs en attente — l’appel des COMPAGNONS à la fin du jeu, le ciblage du groupe, les objectifs d’agence PNJ, les quêtes de faveur et la section « GROUPE » du directeur. **Interception passive ce cycle** (tours indépendants → v2.9).
*   **Le directeur examine l’ensemble du tableau (v2.8) :** une nouvelle section « ÉQUIPEMENT » dans le Ledger (derrière la dépendance cli→provenance de l’équipement), une bande-annonce finale « RÉSUMÉ DU DIRECTEUR », les sections « MARKET OVERVIEW + GROUPE » sont désormais alimentées par des producteurs en direct, et la stabilité du district + le ton économique dans la section « DISTRICTS » à la fin du jeu.
*   **L’autre moitié de l’économie (v2.9) :** un verbe « acheter » complète la boucle — les stocks des marchands sont proposés par district en fonction de la catégorie d’approvisionnement (le niveau d’approvisionnement *est* le signal de réapprovisionnement), et leur prix est fixé via le même pipeline `computeItemValue` que « vendre », ainsi qu’une marge d’achat/vente afin qu’il n’y ait pas de transaction sans risque. Et l’artisanat prend vie : `createCraftingCore` enregistre les tableaux de recettes créés, ce qui active les sections « MATÉRIAUX + RECETTES » du directeur qui étaient inactives auparavant.
*   **Les compagnons effectuent leurs propres tours (v2.9) :** le seuil d’interception passive de la version 2.8 devient le plafond — les compagnons recrutés agissent indépendamment à chaque tour grâce au conseiller `selectBestAction` précédemment inutilisé, avec un biais de combat par rôle afin qu’un combattant et un érudit se battent différemment, l’interception entre compagnons et la santé du groupe sont affichées dans la section « GROUPE » du directeur. Les groupes sans compagnon restent identiques (la porte du groupe vide préserve le rejeu hérité avec la graine 0).
*   **La couche sociale, connectée de bout en bout (v2.9) :** quatre verbes d’influence — « corrompre », « intimider », « solliciter », « semer » (rumeur) — écrivent des valeurs globales réelles de réputation / alerte / chaleur qui affectent les prix et les portes de faction déjà lues, et « semer » active tout le module rumeurs-joueur + la section « RUMEURS SUR VOUS » du directeur. L’*économie* d’influence qui les finance est également connectée : compléter une opportunité accorde désormais l’influence qu’elle a toujours décrite, de sorte que les verbes sont réellement disponibles pendant le jeu.
*   **Opportunités, cycle de vie complet (v2.9) :** un générateur par tour propose des contrats/primes/faveurs évalués en fonction de l’état du monde actuel ; vous « acceptez », puis « complétez » ou « abandonnez »; ignorer une opportunité jusqu’à sa date limite a désormais des conséquences (conséquences de l’expiration), et compléter la faveur d’un compagnon modifie le moral de ce compagnon. Les arcs de montée en puissance et de marchand-prince à la fin du jeu lisent les opportunités que vous avez réellement résolues.
*   **Parité du contenu dans les 10 mondes initiaux (v2.9) :** le câblage de l’équipement, les quêtes, les compagnons recrutables et une somme d’argent de départ ont été déployés dans tous les mondes qui en étaient dépourvus — les 10 mondes partagent désormais une surface de fonctionnalités uniforme et entièrement éclairée (l’équipement était réservé aux gladiateurs ; les quêtes étaient uniquement pour le fantastique/les zombies ; cinq mondes ont déployé « recruter » sans personne à recruter). De plus, un validateur de contenu structurel qui détecte une erreur typographique dans l’ID d’un élément sur toutes les surfaces de référence, et des emplacements d’enregistrement avec plusieurs points de contrôle avec `--checkpoint`/`--list-checkpoints`.
*   **PNJ vivants, réellement vivants (v3.0) :** le producteur d’agence PNJ persistant active la section « PERSONNES » du directeur — les PNJ nommés (un personnage de l’histoire créé par monde initial, plus tous les compagnons que vous recrutez) ont des objectifs, des relations de confiance/peur/avidité/loyauté, un registre d’obligations et des chaînes de conséquences. `runNpcAgencyTick` s’exécute à chaque tour, de sorte qu’un monde sans PNJ nommés reste identique au rejeu hérité. L’activation du producteur a également activé les points de rupture de départ de la faveur des compagnons, deux règles d’apparition d’opportunités dormantes (objectif PNJ + obligation) et les profils/obligations PNJ à la fin du jeu — le câblage a été testé et s’est avéré correct mais inactif dans le contenu déployé jusqu’à ce qu’un audit de phase 9 l’ait détecté, de sorte que la correction déploie un PNJ nommé créé dans chaque monde initial.
*   **La surface sociale complète (v3.0) :** les quatre verbes d’influence deviennent vingt-cinq — les groupes diplomatie et sabotage s’enregistrent (21 autres sous-verbes), ce qui active les réactions de compagnon précédemment inactives « diplomatie-influence » / « sabotage-influence »; 19 d’entre elles apparaissent dans le menu numéroté (abordable + délai de récupération + réputation). Les conditions et les effets du dialogue lisent et écrivent désormais l’état social (influence / réputation / relation PNJ). Et des revenus d’influence passifs (`tickLeverage` / `computeLeverageGains`) versent de l’influence à partir de la réputation et accordent une faveur / un chantage / une légitimité à partir de l’expérience et des étapes importantes — de sorte que la couche sociale rapporte *entre* les opportunités, et pas seulement lors de leur achèvement.
*   **Économie au goût du genre (v3.0) :** les stocks des marchands et les recettes d’artisanat résolvent désormais les tables de genre par monde initial (sept des dix mondes initiaux contiennent du contenu de genre créé ; trois reviennent à l’universel, honnêtement) — dans les mécanismes d’achat/d’artisanat, l’affichage du menu numéroté et la section « RECETTES » du directeur, le tout est lié à la même clé de règle afin que l’affichage et les mécanismes soient en accord. `réparer` et `modifier` sont désormais des lignes dans le menu numéroté (appariement article × recette), et les opportunités « escorte » apparaissent sur une porte d’entrée pour un voyage de protection dans un district dangereux.
*   **La fin du jeu lit l’influence que vous avez gagnée (v3.0) :** les fins des campagnes « victoire », « maître marionnettiste » et « retraite tranquille » — qui étaient longtemps bloquées sur l’influence / le chantage / la légitimité que la couche de fin du jeu lisait comme étant un zéro codé en dur — sont désormais accessibles grâce au véritable magasin d’influence que toute l’économie sociale écrit. Le départ des compagnons est également accessible, via les points de rupture de l’agence PNJ et une valeur minimale de moral.
*   **CLI de développement `audit-content` (v3.0) :** une commande d’audit de contenu pour les développeurs (frère de `validate`, distinct du Ledger du directeur auquel le joueur a accès) qui exécute les six formateurs de directeur des événements / boss / combats sur un pack.
*   `ai-rpg-engine create-starter <name>` — prépare un nouveau jeu (autonome, s’exécute en dehors du monorepositoire) ; commandes `validate` + `scaffold` pour le contenu ; charge les packs à partir de JSON.
