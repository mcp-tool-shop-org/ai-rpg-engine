...
Les deux structures de base sont complètes : **8 356 tests sur 386 fichiers**, les 12 éléments de départ sur `buildCombatStack` et `buildWorldStack`, une relecture déterministe et identique au niveau des octets à partir des graines imprimées, une évaluation complète des décisions de l’IA et une interface en ligne de commande qui permet de créer, d’exécuter, de valider et d’inspecter. L’arc v3.x a donné vie au monde (PNJ nommés, la surface sociale à 25 verbes, économies de genre — v3.0 à v3.1), a placé les actifs appartenant aux joueurs sur le testnet XRPL en tant que canal secondaire facultatif (v3.2 à v3.4), a créé deux éléments de départ axés sur le système et les a transformés en outils d’amélioration du moteur (v3.5 à v3.6), a affiné et renforcé la couche stratégique jusqu’à ce que les conséquences laissent des marques réelles (v3.7 à v3.8), a fourni aux hôtes la surface du moteur dont ils avaient besoin pour l’attacher à Godot (v3.8.1), a bouclé la boucle de création afin qu’une session en studio ou un simple ensemble de données JSON produise un monde jouable de bout en bout (v3.9), a placé toute la couche stratégique dans les sens du joueur (v3.10) et a **affiné la profondeur que ces sens atteignent désormais : récompense d’entrée, repli en tant que résultat, adhésion à une faction dans trois lieux, `/build` survivant à un crash (v3.11)**.
...
La version 3.11 a clôturé la phase d’ajustement et d’approfondissement. Ce qui reste est un travail ultérieur, et non des éléments résiduels de cette phase :

- Un environnement ambiant de famille prospère (le pont tonal et le système de répartition par zone ; CORE n’a toujours pas de base prospère)
...
- Remplacements de formules sérialisables : ajustement des formules par profil (en attente d’un DSL de formules ; les profils contiennent aujourd’hui des mappages de statistiques, et non des fermetures)
