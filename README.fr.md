...
Les deux structures de composition sont terminées — **8 223 tests sur 385 fichiers**, les 12 éléments de départ sur `buildCombatStack` **et** `buildWorldStack`, relecture déterministe et identique au niveau des octets à partir des graines imprimées, évaluation complète des décisions de l’IA et une interface en ligne de commande qui crée, exécute, valide et inspecte. L’arc v3.x a donné vie au monde (PNJ nommés, la surface sociale à 25 verbes, économies de genre — v3.0–v3.1), a placé les actifs appartenant aux joueurs sur le testnet XRPL en tant que canal secondaire facultatif (v3.2–v3.4), a créé deux éléments de départ axés sur le système et les a transformés en outils d’amélioration du moteur (v3.5–v3.6), a affiné et renforcé la couche stratégique jusqu’à ce que les conséquences laissent des marques réelles (v3.7–v3.8), a fourni aux hôtes la surface du moteur avec les éléments nécessaires pour l’attacher à Godot (v3.8.1), a fermé la boucle de création afin qu’une session en studio ou un simple ensemble JSON produise un monde jouable de bout en bout (v3.9), et **a placé toute la couche stratégique dans les sens du joueur — indices, groupe, victoire, musique axée sur l’ambiance et le déroulement — prouvé par une session de jeu de bout en bout (v3.10)**.
...
### Prochainement (le cycle d’ajustement et d’approfondissement)

La v3.10 a fourni la surface utilisateur ; ce qu’elle a enregistré en sortant définit le cycle suivant :

- **Réajuster l’accès de `bounty` (le P1 nommé)** — le nettoyage du récepteur de départ a modifié le flux d’événements déterministe et la fenêtre de récompense à quatre conditions de black-flag-requiem ne se produit plus dans une session naturelle ; le mécanisme se déroule de manière synthétique, et `faction-summons` (toute sa chaîne de conséquences) attend derrière — un passage d’équilibrage dédié à un seul levier avec le système de portée comme porte
- **Une fin honnête pour un combat que personne n’a gagné** — fuir un combat qui vide ensuite la zone ne déclenche aucun événement aujourd’hui (règle : la retraite n’est pas la victoire) ; une conception d’événement terminé (retrait/abandon) portant `outcome` avec sa propre particularité et sa propre sémantique de mémoire
- **Les compagnons se souviennent d’où ils viennent** — le recrutement réécrit actuellement un compagnon sur la faction partagée du groupe, de sorte que « la guilde dont ils viennent vous écoute » est inaccessible ; la préservation d’une faction d’origine sur `CompanionState` débloque le bonus de levier de la route de faction tel que conçu
- **Cognition de faction sans l’étape d’enregistrement** — la propagation des rumeurs et le cadrage de l’observateur dépendent toujours fortement d’un enregistrement explicite `createFactionCognition` qu’aucun ensemble livré n’effectue ; la dérivation de l’adhésion à partir de `entity.faction` créé (et le routage de l’hostilité de l’observateur via `affiliationOf` au lieu de la simple inégalité de faction) achève ce que le repli d’identité de la v3.10 a commencé
- **Variété de musique de zone et extensibilité de l’ensemble** — le pont tonal est d’abord déterministe (même zone, même élément) ; un roulement déterministe par zone et une surface tonalité→ambiance créable par l’ensemble sont les prochaines étapes prévues, ainsi que le lit ambiant prospère manquant
- **Lots de construction en cours qui survivent à un plantage** — la v3.10 avertit à la sortie ; la sérialisation de `BuildState`/`TuningState` (y compris les écritures en attente) dans le fichier de session est la solution complète
- Multijoueur — deux joueurs *humains* partageant un monde (une couche de mise en réseau, délibérément reportée ; les profils partagés à contrôleur unique sont disponibles aujourd’hui sous la forme de [`shared-profiles.ts`](docs/examples/shared-profiles.ts))
- Substitutions de formules sérialisables — ajustement de la formule par profil (bloqué sur un DSL de formule ; les profils contiennent aujourd’hui des mappages de statistiques, et non des fermetures)
