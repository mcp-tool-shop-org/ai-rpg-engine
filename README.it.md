...
Entrambe le strutture di base sono complete: **8223 test su 385 file**, tutti i 12 personaggi iniziali su `buildCombatStack` **e** `buildWorldStack`, riproduzione deterministica e identica byte per byte con seed predefiniti, valutazione completa delle decisioni dell'IA e una CLI che crea, esegue, convalida e ispeziona. L'arco v3.x ha dato vita al mondo (personaggi non giocanti con nomi, la superficie sociale con 25 verbi, economie di genere — v3.0–v3.1), ha inserito risorse di proprietà del giocatore sulla testnet XRPL come canale secondario opzionale (v3.2–v3.4), ha creato due personaggi iniziali innovativi e li ha trasformati in strumenti per ottimizzare il motore (v3.5–v3.6), ha reso più dinamico e complesso lo strato strategico fino a quando le conseguenze lasciano segni reali (v3.7–v3.8), ha fornito agli host la superficie del motore con le funzionalità necessarie per Godot (v3.8.1), ha chiuso il ciclo di creazione in modo che una sessione di studio o un semplice pacchetto JSON producano un mondo giocabile dall'inizio alla fine (v3.9) e **ha portato l'intero strato strategico nelle sensazioni del giocatore: indizi, gruppo, vittoria, musica guidata dall'umore e la trama — dimostrato da una sessione di gioco end-to-end (v3.10)**.
...
### Prossimo (il ciclo di ottimizzazione e approfondimento)

La v3.10 ha fornito la superficie per l'utente finale; ciò che ha registrato durante il processo definisce il ciclo successivo:

- **Ricalibrare l'avvio di `bounty` (il personaggio P1 con nome)**: la pulizia del listener iniziale ha modificato il flusso di eventi deterministico e la finestra di opportunità a quattro condizioni di black-flag-requiem non si attiva più in una sessione naturale; il meccanismo funziona in modo sintetico e `faction-summons` (l'intera catena di conseguenze) attende dietro di esso: un bilanciamento dedicato con una singola leva, con il sistema di raggiungibilità come filtro.
- **Una fine onesta per una battaglia che nessuno ha vinto**: fuggire da una battaglia che svuota la zona non attiva alcun evento oggi (regola: la ritirata non è una vittoria); un design di incontro con `outcome` che indica la fine (ritirata/abbandono) con le proprie caratteristiche e semantiche di memoria.
- **I compagni ricordano da dove vengono**: il reclutamento riscrive attualmente un compagno sulla fazione condivisa del gruppo, quindi "la gilda da cui provengono ti ascolta" è irraggiungibile; preservare una fazione di origine su `CompanionState` sblocca il bonus di leva della fazione come previsto.
- **Cognizione della fazione senza il passaggio di registrazione**: la propagazione delle voci e l'inquadramento degli osservatori dipendono ancora fortemente da una registrazione esplicita `createFactionCognition` che nessun pacchetto fornito esegue; derivare l'appartenenza da `entity.faction` creato (e indirizzare l'ostilità dell'osservatore attraverso `affiliationOf` invece della semplice disuguaglianza di fazione) completa ciò che il fallback di identità della v3.10 ha iniziato.
- **Varietà della musica di zona ed estensibilità del pacchetto**: il ponte tonale è deterministico (stessa zona, stessa base); un roll deterministico per zona e una superficie tono→umore modificabile nel pacchetto sono i passaggi successivi previsti, insieme alla base ambientale di famiglia prospera mancante.
- **Batch di build in corso che sopravvivono a un crash**: la v3.10 avvisa all'uscita; la serializzazione di `BuildState`/`TuningState` (inclusi gli scritture in staging) nel file di sessione è la soluzione completa.
- Multiplayer: due giocatori *umani* che condividono un mondo (un livello di rete, volutamente differito; i profili condivisi a controller singolo sono disponibili oggi come [`shared-profiles.ts`](docs/examples/shared-profiles.ts)).
- Override di formule serializzabili: ottimizzazione delle formule per profilo (bloccata su un DSL di formule; i profili contengono oggi mappature di statistiche, non chiusure).
