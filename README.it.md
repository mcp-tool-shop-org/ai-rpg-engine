...
- Una **libreria di moduli** — oltre 30 moduli del motore che coprono combattimento, percezione, cognizione, fazioni, voci, spostamenti, compagni e altro ancora.
...
*   Motore principale: stato del mondo, eventi, azioni, cicli di gioco, replay — stabile dalla versione 1.0; replay deterministico e identico byte per byte (contatore ID per istanza, generatore di numeri casuali con seme).
...
*   **Ciclo di gioco eseguibile (`run`) (v2.6):** il gioco finale è reale, non una demo — i nemici agiscono in base ai propri profili di intelligenza artificiale (`aggressivo`/`cauto`/`territoriale`/`calcolatore`), un combattimento termina con vittoria o sconfitta, puoi salvare e riprendere, e le abilità e l'esperienza sono nel menu delle azioni. `run <path>` carica un gioco che hai creato. Interfaccia utente finale composta con HUD facilmente consultabile e colori accessibili (rispetta `NO_COLOR` / non-TTY).
...
*   **Un motivo per tornare (v2.7):** un ciclo di missioni minimo sullo schema già implementato da tempo — le missioni offrono opportunità in base ai trigger, tracciano gli obiettivi di uccisione/raggiungimento/progresso e forniscono esperienza e oggetti una sola volta; quattro missioni create, una schermata "Diario", elementi delle missioni nella narrazione del ciclo.
*   **L'equipaggiamento influenza il combattimento (v2.7):** `equip`/`unequip` modificano i valori reali attraverso lo strato di stato che le formule di combattimento già leggono — nessuna modifica al codice di combattimento; tridente e rete da gladiatore sono collegati end-to-end con un delta di probabilità di colpo testato.
...
*   **`buildWorldStack()` (v2.7):** la struttura di composizione strategica insieme a `buildCombatStack()` — una singola chiamata assembla ambiente, fazioni, voci, distretti, conseguenze della sconfitta, incontri e missioni; più la schermata "Registro del direttore" per la strategia, un ispettore di simulazione con `AI_RPG_DEBUG=1`, `inspect-save` protetto dalle stesse autorizzazioni di Continua e una connessione di migrazione dei moduli salvati nel percorso di ripristino.
...
*   **Parità dei contenuti in tutti i dieci mondi iniziali (v2.9):** collegamento dell'equipaggiamento, missioni, compagni reclutabili e un saldo di monete iniziale implementati in ogni mondo iniziale che ne era privo — i dieci mondi ora condividono una superficie di funzionalità uniforme e completamente illuminata (l'equipaggiamento era solo da gladiatore; le missioni erano solo fantasy/zombie; cinque mondi hanno implementato "recluta" senza nessuno da reclutare). Inoltre, un validatore strutturale dei contenuti che rileva un ID oggetto digitato male in ogni superficie di riferimento e slot di salvataggio con più checkpoint con `--checkpoint`/`--list-checkpoints`.
*   **NPC dinamici, effettivamente vivi (v3.0):** il produttore persistente dell'IA NPC illumina la sezione "Persone" del direttore — gli NPC nominati (un personaggio della storia creato per mondo iniziale, più ogni compagno che recluti) hanno obiettivi, relazioni di fiducia/paura/avidità/lealtà, un registro degli obblighi e catene di conseguenze. `runNpcAgencyTick` viene eseguito a ogni ciclo, con restrizioni in modo che un mondo senza NPC nominati rimanga identico byte per byte al replay legacy. L'illuminazione del produttore ha anche illuminato i punti di interruzione della partenza dei compagni dovuti alla morale e due regole di generazione delle opportunità latenti (obiettivo NPC + obbligo) e gli NPCProfiles/NPCobligations dell'arco finale — il collegamento è stato testato ed era funzionante, ma inattivo nei contenuti implementati fino a quando un audit di fase 9 non lo ha rilevato, quindi la correzione implementa un NPC nominato creato in ogni mondo iniziale.
*   **L'intera superficie sociale (v3.0):** i quattro verbi di leva diventano venticinque — i gruppi di diplomazia e sabotaggio si registrano (altri 21 sottoverbi), illuminando le reazioni dei compagni precedentemente oscure `leverage-diplomacy` / `leverage-sabotage`; diciannove appaiono nel menu numerato (costo + tempo di ricarica + reputazione limitati). Le condizioni e gli effetti del dialogo ora leggono e scrivono lo stato sociale (leva / reputazione / relazione NPC). E il reddito passivo della leva (`tickLeverage` / `computeLeverageGains`) fa gocciolare l'influenza dalla reputazione e fornisce favori / ricatti / legittimità dall'esperienza e dalle pietre miliari — quindi il livello sociale si guadagna *tra* le opportunità, non solo al completamento.
*   **Economia con sapore di genere (v3.0):** l'inventario del mercante e le ricette di creazione ora risolvono le tabelle di genere per mondo iniziale (sette dei dieci mondi iniziali hanno contenuti di genere creati; tre tornano a un valore universale), attraverso i meccanismi di acquisto/creazione, la visualizzazione nel menu numerato e la sezione "Ricette" del direttore, tutti collegati dalla stessa chiave dell'insieme di regole in modo che la visualizzazione e la meccanica concordino. "Riparare" e "modificare" sono ora righe del menu numerato (accoppiamento articolo × ricetta) e le opportunità "scorta" vengono generate su un gate di viaggio protettivo in un distretto pericoloso.
*   **Il finale legge la leva che hai guadagnato (v3.0):** i finali delle campagne "vittoria", "burattinaio" e "pensione tranquilla" — precedentemente limitati all'influenza / ricatto / legittimità che il livello finale leggeva come zero hardcoded — sono ora raggiungibili attraverso il vero archivio di leva che l'intera economia sociale scrive. Anche la partenza del compagno è raggiungibile, tramite i punti di interruzione dell'IA NPC e un fallback sul limite minimo della morale.
*   **CLI di sviluppo `audit-content` (v3.0):** un comando di audit dei contenuti per sviluppatori (fratello di "valida", distinto dal registro del direttore rivolto al giocatore) che esegue i sei formattatori direttori di incontri / boss / combattimento su un pacchetto.
*   **Fornitura iniziale con sapore di genere — l'apertura della v3.0, fornita (v3.1):** `economyGenre` collega la chiave dell'insieme di regole di ciascun mondo iniziale attraverso `buildWorldStack` → `createEconomyCore`, in modo che un distretto ora generi il profilo del suo genere `GENRE_SUPPLY_DEFAULTS` (cyberpunk ha molti componenti / merce di contrabbando, fantasy ha poche medicine) invece di una base universale piatta — la fornitura iniziale che la tonalità del mercato del direttore e gli input del finale già leggono. Sette dei dieci mondi iniziali hanno un profilo di genere; tre tornano al valore predefinito, onestamente. Un campo separato da `tradeGenre` / `craftingGenre` in modo che i tre possano divergere in seguito.
*   **La superficie sociale, completa (v3.1):** "nega" e "seppellisci lo scandalo" — la coppia di manipolazione delle voci che prende di mira una voce esistente per ID anziché una fazione — raggiungono il menu numerato tramite una dimensione di accoppiamento della voce di destinazione, chiudendo la superficie dei ventuno verbi (19 → 21 visualizzati).
*   **Dialogo `obligation-exists`, collegato e raggiungibile (v3.1):** la condizione del dialogo legge il registro degli obblighi persistenti di un NPC nominato (`getPersistedNpcObligations`) — Fratello Aldric della fantasia, una volta che ti deve un favore attraverso il normale gioco dell'IA NPC, sblocca una scelta "chiedi un favore" — un vero gate in cui la v3.0 aveva lasciato uno stub sempre vero silenzioso (un audit di sessione giocata di fase 9 ha dimostrato che è raggiungibile in un gioco reale, non solo nei test unitari).
*   **Riparazione con sapore di genere (v3.1):** ogni mondo iniziale con contenuto di genere crea una ricetta "riparazione" caratteristica nella sua tabella di genere (fantasia `repair-rune-mend`, cyberpunk `repair-nanite-weld`, …), visualizzata tramite `getAvailableRecipes` — la riparazione ora ha un sapore, non è solo universale.
*   `ai-rpg-engine create-starter <name>` — crea un nuovo gioco (autonomo, eseguito al di fuori del monorepo); comandi "valida" e "crea" per i contenuti; carica i pacchetti da JSON.
*   Modello iniziale pubblicato su npm (`@ai-rpg-engine/starter-template`).
*   Suite di test completa: **5512 test** (deterministici tra esecuzioni ripetute; i file di test sono controllati in CI; il limite di copertura è applicato).
