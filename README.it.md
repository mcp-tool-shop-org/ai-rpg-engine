...
Lo script comunica con un daemon [Ollama](https://ollama.com) locale: eseguire prima `ollama serve` e `ollama pull qwen2.5-coder`. È del tutto facoltativo; il motore e il ciclo `run` non necessitano di una connessione di rete.

Un'immagine container viene pubblicata su GHCR come `ghcr.io/mcp-tool-shop-org/ai-rpg-engine` per l'integrazione continua (CI) e le esecuzioni in ambiente isolato.
...
`@ai-rpg-engine/ledger-adapter` è un pacchetto **facoltativo** che associa lo strato di gioco **di proprietà del giocatore e scambiabile** — il saldo `coin` e l'inventario di oggetti consumabili che i verbi `buy`/`sell` di `trade-core` già gestiscono — alla **testnet XRPL**, in modo che tali asset possano essere supportati da token reali registrati sulla blockchain e regolati nei punti di controllo. L'assenza dell'adattatore corrisponde esattamente al motore offline disponibile oggi.
...
- Nulla in `@ai-rpg-engine/core` o `@ai-rpg-engine/modules` lo importa (la sua unica dipendenza dal motore è una libreria utilizzata in fase di compilazione `import type`).
- **Un'esecuzione è identica con o senza.** Un test del firewall esegue il ciclo reale del commerciante `starter-pirate` `createGame()` su due motori: uno con l'adattatore abilitato e che effettua la regolazione in un punto di controllo, e verifica che i due mondi siano profondamente equivalenti. La riproduzione con seme 0 non viene modificata.
...
| [Plug-in Profiles](site/src/content/docs/handbook/59-plugin-profiles.md) | Risoluzione delle regole per entità: combattimento con stili di gioco misti, `applyProfile`, modelli di profilo, l'interfaccia a riga di comando (CLI) `profile`. |
