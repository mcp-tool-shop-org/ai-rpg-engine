...
**哪些部分存在缺陷或不完整：**
- 人工智能世界构建工作室（Ollama 层）的测试不如模拟核心充分，并且需要本地 Ollama 守护进程；它完全是可选的——引擎和 `run` 循环不需要网络连接。
- 叙事/音频堆栈会生成确定性的音频命令，但**没有终端音频后端**——没有任何声音播放；这些命令是一个集成钩子，用于 GUI/Web 嵌入器。
- 多人游戏（两个人类玩家共享一个世界）**尚未构建**——它是一个网络层，有意不在本次开发范围内；目前的配置针对单个控制器。
- `replay --replay` 会恢复存档而不是重新模拟——并且在 v2.9 版本之后，这就是**最终方向**，而不是暂缓：`Engine.serialize()` 已经是一种经过验证的完整状态快照，而重新模拟则需要跟踪存在于操作日志之外的世界时间点/遭遇状态。v2.9 版本通过这种经过验证的恢复路径提供了多检查点的存档插槽；真正的基于事件的重模拟尚未计划。
- 仍然有一些 v3.0 版本中未完全实现的功能：区域**初始物资**仍然从通用基线生成，而不是从每个类型的 `GENRE_SUPPLY_DEFAULTS`（可购买的物品和配方是按类型分类的——物资将在 v3.1 版本中推出，具体方案已确定）；没有起始角色拥有特定类型的*修理*配方（修理功能在所有地方都可用，只是不具有类型特征）；并且二十一个社交子动词中的两个（`deny`、`bury-scandal`）需要在达到编号菜单之前进行谣言目标配对。
