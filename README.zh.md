...
**已实现并经过测试的功能：**
- 核心运行时：世界状态、事件、动作、回合、重播——自 v1.0 起稳定；确定性的字节级重播（每个实例的 ID 计数器，种子随机数生成器）
- 战斗系统：5 个动作、4 种战斗状态、4 种交战状态、伙伴拦截、失败流程、AI 策略
- 能力：消耗、冷却时间、属性检查、类型化的效果、11 标签的状态词汇表、AI 感知的选择
- **队伍战斗 (v2.4)：** 盟友目标（治疗/增益/复活）、友方/敌方区域伤害过滤、目标选择器——一个治疗者可以治疗队友；敌方区域伤害不会对盟友造成伤害
- **状态效果 (v2.4)：** 被动属性修正值影响战斗，确定性的持续伤害/持续治疗基于回合计数器，深度限制的反应触发（荆棘/反射）
- **插件配置文件——每个实体的规则解析 (v2.5)：** 一个“力量”战士和一个“意志”神秘者在一场战斗中解决冲突，各自读取通过自己的映射表获取的属性。`RuleProfile` + `WorldState.ruleProfiles` + `EntityState.ruleProfileId`；`applyProfile()` 附加一个配置文件（属性映射、资源池、每个实体的能力）；`buildProfile()`、`validateProfileSet()`（拒绝重复 ID），10 个基于初始模板的配置，以及一个 `profile` CLI 命令
- **可玩“运行”循环 (v2.6)：** 最终游戏是真实的，而不是演示——敌人根据自己的 AI 意图配置文件（“积极”、“谨慎”、“领地意识”、“计算”）行动，战斗以胜利或失败结束，您可以保存并恢复进度，能力和经验值显示在动作菜单中。`run <path>` 加载您构建的游戏。由一个易于查看的 HUD 和可访问的颜色方案组成的终端 UI（支持 `NO_COLOR`/非 TTY）。
...
- 内容创作的标签分类和验证工具
- **世界会做出反应 (v2.7)：** 击杀会增加热度并降低区域安全性；每回合的世界事件会产生隐藏的压力，这些压力会以谣言的形式出现（“你听到了一些耳语……”），升级，并在一段时间后消失，从而产生后果；在所有 10 个初始世界中，大约有 30 个已编写的遭遇组合会在进入区域时触发——基于种子的确定性，更危险的区域会产生更多敌人，Boss 的场景受到保护。
- **一个回归的理由 (v2.7)：** 在长期存在的架构上构建了一个最小的任务循环——任务在触发器处提供，跟踪击杀/到达/进度目标，并精确地一次性奖励经验值和物品；四个已编写的任务、一个“日志”屏幕，以及回合叙述中的任务环节。
- **装备影响战斗 (v2.7)：** `equip`/`unequip` 会将实际数值传递到状态层，而战斗公式已经读取了这些数据——无需更改任何战斗代码；角斗士的三叉戟和渔网从头到尾都进行了连接，并使用测试来验证命中率的变化。
- **基于种子的运行 (v2.7)：** 每个新的会话都会打印其种子以及确切的重播命令；`--seed <n>` 会逐字节地重现一个会话；战斗、抵抗、能力和策略的所有掷骰都使用世界种子——并且结局会读取您实际玩过的游戏（实时的热度、压力、派系积累、玩家等级）。
- **`buildWorldStack()` (v2.7)：** 战略组合框架，与 `buildCombatStack()` 并行——一个调用组装环境、派系、谣言、区域、失败后果、遭遇和任务；此外还有“总监日志”策略屏幕、一个 `AI_RPG_DEBUG=1` 模拟检查器、一个由与“继续”相同的权限控制的 `inspect-save`，以及已发布恢复路径上的模块保存迁移接口。
- **对动态经济采取行动 (v2.8)：** `createEconomyCore` 在加载包时为每个区域创建一个经济体系，并在每个回合中对其进行更新；一个新的“出售”动词通过 `computeItemValue`（稀缺性/派系/来源/走私品）来确定战利品的价值，并改变当地的供应。一个编写代码点亮了五个系统，这些系统在 v2.7 中以隐藏的形式存在——总监的市场概览 + 派系评分、最终商人王子剧情和崩溃触发器，以及四种经济压力类型。**本周期仅允许出售（购买→v2.9）**
- **伙伴 (v2.8)：** 一个“招募”动词构建一个队伍——状态、标签和派系，因此伙伴会*与您一起*战斗；伙伴的战斗依赖于战斗核心的拦截机制（在 `isAlly` 设置之前是隐藏的），伙伴会对士气做出反应并可能离开，招募会激活七个等待使用的功能——最终的“伙伴”名单、队伍目标、NPC 代理目标、好感任务以及总监的“队伍”部分。**本周期仅允许被动拦截（独立的回合→v2.9）**
- **总监可以查看整个棋盘 (v2.8)：** 一个新的装备日志部分（依赖于 CLI 到装备来源），一个总监总结最终预告片，市场概览 + 队伍部分现在从实时生成器获取数据，以及区域稳定性和经济基调在最终的“区域”部分中显示。
- `ai-rpg-engine create-starter <name>`——构建一个新的游戏（独立，可以在单仓库之外运行）；`validate` + `scaffold` 内容命令；从 JSON 加载包。
- 已发布的初始模板在 npm 上 (`@ai-rpg-engine/starter-template`)
- 完整的测试套件：**4975 个测试**（在重复运行中具有确定性；测试文件在 CI 中进行类型检查；覆盖率由强制执行机制控制）
...
两个组合框架都已完成——273 个文件中共有 4975 个测试，所有 10 个初始世界都在 `buildCombatStack` **和** `buildWorldStack` 上，基于打印的种子的确定性字节级重播，完整的 AI 决策评分，以及一个可以构建、运行、验证和检查的游戏。**v2.8 允许您对 v2.7 构建的动态世界采取行动：一个您可以与“出售”动词一起使用的贸易经济，您可以招募并与之并肩作战的伙伴，以及一个总监日志，它可以查看整个棋盘——每个系统都是一个单独的代码点，它激活了已经以隐藏形式存在的消费者。**

**近期发布版本（v2.4.0–v2.8.0）：**
- v2.4.0 — 团队战斗（针对盟友/治疗/增益/复活，友方/敌方范围攻击），状态效果系统（修正值 + 持续伤害/持续回复 + 反应触发器），插件配置阶段 1，内容 `validate`/`scaffold` CLI
- v2.5.0 — 每个实体的规则解析（混合战斗风格），`applyProfile` 加载器 + 每个实体的能力，配置文件模板 + `profile` CLI，以及完整的生命值调整
- v2.6.0 — `run` 命令真正成为一个游戏：敌人根据自己的 AI 配置行动，胜/负判定，保存/恢复进度，菜单中的能力和经验值，`ai` 工作室 bin 文件，以及叙事堆栈
- v2.7.0 — 世界会产生反应，并且有理由再次探索：热度 → 压力 → 叙述性的后果，区域入口遭遇战，任务循环 + 日记，战斗中的装备，可重复进行的关卡，实时游戏结束输入，`buildWorldStack`，导演日志，以及保存数据迁移接口
- **v2.8.0 — 对你所处的世界采取行动：实时的交易经济 + `sell` 动词，你可以招募并一同作战的伙伴，以及导演日志，它会分析整个游戏系统——每个系统都有一个写入线程，大约有 12 个已发布的游戏**

### 下一步（v2.8 的框架）

- 购买和商人库存（货币 + 商店物品），以及制作/回收循环——这是交易经济的另一部分
- 完整的伙伴回合制行动（独立的行动，而不仅仅是拦截），以及社交动词——通过杠杆系统进行贿赂/恐吓/散布谣言
- NPC 代理和机会核心生成器，为导演日志中的“人物”部分和伙伴士气/好感度提供内容；装备内容超越角斗士
- `--replay` 重模拟与世界状态模块的同步，以及剩余的导演格式化界面
- 多人游戏——两个*人类*玩家共享一个世界（网络层，有意延迟；单控制器共享配置今天发布为 [`shared-profiles.ts`](docs/examples/shared-profiles.ts)）
- 可序列化的公式覆盖——每个配置的公式调整（受限于公式 DSL；配置文件现在包含状态映射，而不是闭包）
- API 文档同步——确保每个手册页面都反映 v2.7 的 API
