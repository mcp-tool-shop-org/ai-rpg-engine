...
**レジャー上にあるもの。** `coin` → トラストライン上の発行通貨IOU；消費可能アイテム → 換金可能なトークン；チェックポイントでの純取引デルタ → **XLS-85トークンスキュー** を介した決済済み転送。ユニークな装備をNFTとして扱うことは、意図的に後で行うものです。抽象的な地区経済 (`economy-core`) は変更されません — 純粋なシミュレーションのままです。
