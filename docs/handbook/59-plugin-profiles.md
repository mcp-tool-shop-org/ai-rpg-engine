...
This is **CR-1: per-entity rule resolution**. The runnable proof is [`docs/examples/mixed-party.ts`](../examples/mixed-party.ts) and [`docs/examples/shared-profiles.ts`](../examples/shared-profiles.ts), both type-checked and behaviour-tested in CI.
...
Everything it writes is plain data, so it rides `WorldStore.serialize()` with the rest of world state and survives save/load unchanged (see the byte-identical-replay assertion in [`examples.test.ts`](../examples/examples.test.ts)).
...
From [`docs/examples/mixed-party.ts`](../examples/mixed-party.ts) — a brute fighter and a cunning mystic in one fight, plus a wolf that never heard of profiles.
...
- Runnable examples: [`mixed-party.ts`](../examples/mixed-party.ts), [`shared-profiles.ts`](../examples/shared-profiles.ts)
