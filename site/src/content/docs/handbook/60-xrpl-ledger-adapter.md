---
title: "Chapter 60 — The XRPL Ledger Adapter"
description: "Opt-in XRPL testnet settlement for a game's player-owned tradeable layer — entirely outside the deterministic core."
sidebar:
  order: 60
---
...
**player-owned tradeable layer** — the `coin` balance and the consumable
inventory that `trade-core`'s `buy`/`sell` verbs already move — to the **XRPL
testnet**, so those assets can be backed by real on-ledger tokens and settled at
checkpoints. It is entirely opt-in: an absent adapter is exactly the offline
