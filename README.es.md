...
El estudio se comunica con un demonio local de [Ollama](https://ollama.com); ejecute primero `ollama serve` y `ollama pull qwen2.5-coder`. Es totalmente opcional; el motor y el bucle `run` no necesitan conexión de red.
...
| [`@ai-rpg-engine/starter-merchant`](packages/starter-merchant) | Kit inicial mercantil: el paquete de referencia para el adaptador del libro mayor, sin ninguna dependencia de este. |
