...
- Un **estudio de diseño de IA** (opcional): estructura, crítica, análisis de equilibrio, ajuste, experimentos a través de Ollama.
- Una **capa opcional en la cadena de bloques**: `@ai-rpg-engine/ledger-adapter` respalda la moneda de un juego y los objetos intercambiables con tokens reales de la **red de prueba** XRPL, que se liquidan en puntos de control, completamente fuera del núcleo determinista (opcional; una ejecución es idéntica en bytes sin ella).
- Un **host visual opcional**: `@ai-rpg-engine/sidecar` más el cliente Godot 4 [`ai-rpg-stage`](https://github.com/mcp-tool-shop-org/ai-rpg-stage). El motor aún no genera píxeles. El entorno dibuja un puerto dimétrico de 2:1 y reproduce el audio `felt`. La ocupación sigue siendo un ID de zona.
...
| [Visual Clients](site/src/content/docs/handbook/66-visual-clients.md) | Hosts complementarios, carga útil de "felt", puerto dimétrico de Godot: la presentación puede ser engañosa, pero la ocupación sigue siendo un ID de zona. |
...
La versión 3.12 cerró el contrato de "felt" y el primer host de dibujo. Lo que queda es trabajo de ciclos posteriores, no restos de este ciclo:

- Un entorno de juego con temática de familia próspera (se implementa el puente de tono y el ajuste por zona; CORE aún no tiene un elemento principal próspero)
- Multijugador: dos jugadores *humanos* comparten un mundo (se implementa una capa de red, que se pospuso deliberadamente; hoy se lanzan los perfiles compartidos para un solo controlador como [`shared-profiles.ts`](docs/examples/shared-profiles.ts))
- Anulación de fórmulas serializables: ajuste de fórmulas por perfil (pendiente de un DSL de fórmulas; los perfiles contienen asignaciones de estadísticas, no funciones de cierre)
