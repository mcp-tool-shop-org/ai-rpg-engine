...
- Una **biblioteca de módulos**: más de 30 módulos del motor que abarcan el combate, la percepción, la cognición, las facciones, los rumores, el movimiento, los compañeros y mucho más.
...
- Un **entorno de ejecución de simulación**: ciclos deterministas, registros de acciones reproducibles, generador de números aleatorios con semilla.
...
- Una **capa opcional en la cadena de bloques**: `@ai-rpg-engine/ledger-adapter` respalda la moneda y los objetos intercambiables de un juego con tokens reales de la **red de pruebas** XRPL, que se liquidan en puntos de control, completamente fuera del núcleo determinista (opcional; una ejecución es idéntica sin ella).
...
## Estado actual (v3.4.0)

**Lo que funciona y ha sido probado:**
...
*   **Efectos de estado (v2.4):** los modificadores de estadísticas pasivos afectan al combate, daño por segundo/curación por segundo determinista basado en el contador de ciclos, desencadenantes reactivos con profundidad limitada (espinas/reflejo).
...
*   **El equipo afecta al combate (v2.7):** `equip`/`unequip` mueve números reales a través de la capa de estado que las fórmulas de combate ya leen; cero cambios en el código de combate; el tridente y la red del gladiador están conectados de extremo a extremo con un delta de probabilidad de golpe probado.
...
*   **`buildWorldStack()` (v2.7):** la base estratégica para la composición junto a `buildCombatStack()`: una sola llamada ensambla el entorno, las facciones, los rumores, los distritos, las consecuencias de la derrota, los encuentros y las misiones; además, la pantalla de estrategia **Director's Ledger**, un inspector de simulación con `AI_RPG_DEBUG=1`, `inspect-save` protegido por las mismas autoridades que Continuar y una unión de migración de módulos en la ruta de restauración utilizada.
*   **Actuar sobre la economía viva (v2.8):** `createEconomyCore` inicializa una economía por distrito al cargar el paquete y la actualiza cada ronda; un nuevo verbo `sell` fija el precio del botín a través de `computeItemValue` (escasez / facción / procedencia / contrabando) y cambia el suministro local. Una sola línea de código activó cinco sistemas que se enviaron desactivados en v2.7: la visión general del mercado + la puntuación de las facciones del Director, el arco del comerciante-príncipe al final del juego y el desencadenante de colapso, y cuatro tipos de presión económica. **Solo venta este ciclo** (compra → v2.9).
*   **Compañeros (v2.8):** un verbo `recruit` construye un grupo: estado, etiquetas y facción, por lo que un compañero lucha *contigo*; el combate del compañero se basa en la mecánica de interceptación del núcleo de combate (desactivada hasta que se establece `isAlly`), los compañeros reaccionan con moral y pueden irse, y reclutar activa siete consumidores pendientes: la lista de COMPAÑEROS del final, el objetivo del grupo, las metas de agencia NPC, las misiones de favor y la sección PARTY del Director. **Interceptación pasiva este ciclo** (turnos independientes → v2.9).
*   **El Director lee todo el tablero (v2.8):** una nueva sección EQUIPAMIENTO en el Ledger (detrás de la dependencia cli→procedencia del equipo), un tráiler final del RESUMEN DEL DIRECTOR, las secciones VISIÓN GENERAL DEL MERCADO + GRUPO ahora se alimentan de productores en vivo y la estabilidad del distrito + el tono económico en la sección DISTRITOS del final.
*   **La otra mitad de la economía (v2.9):** un verbo `buy` completa el ciclo: el inventario del comerciante ofrecido por distrito a nivel de categoría de suministro (el nivel de suministro *es* la señal de reabastecimiento), con precios a través de la misma canalización `computeItemValue` que `sell` más un diferencial de compra/venta para que no haya viajes sin riesgo. Y la artesanía cobra vida: `createCraftingCore` registra `salvage`/`craft`/`repair`/`modify` sobre las tablas de recetas creadas, iluminando las secciones MATERIALES + RECETAS del Director que se enviaron desactivadas.
*   **Los compañeros hacen sus propios turnos (v2.9):** el piso de interceptación pasiva de v2.8 se convierte en el techo: los compañeros reclutados actúan independientemente cada ronda a través del asesor `selectBestAction` previamente no utilizado, con un sesgo de combate por rol para que un luchador y un erudito luchen de manera diferente, la interceptación compañero-compañero y la salud del grupo en la línea PARTY del Director. Los paquetes sin compañeros siguen siendo byte idénticos (la puerta de fiesta vacía conserva la repetición heredada de la semilla 0).
*   **La capa social, conectada de extremo a extremo (v2.9):** cuatro verbos de influencia: `bribe`, `intimidate`, `petition`, `seed` (rumor); escriben globales reales de reputación / alerta / calor que intercambian los precios y las puertas de facción que ya leen, y `seed` ilumina todo el módulo de rumores del jugador + la sección RUMORES SOBRE TI del Director. La *economía* de influencia que los financia también está conectada: completar una oportunidad ahora otorga la influencia que siempre narró, por lo que los verbos se pueden obtener genuinamente en el juego.
*   **Oportunidades, el ciclo de vida completo (v2.9):** un generador por ronda ofrece contratos/recompensas/favores puntuados según el estado del mundo en vivo; `accept`, luego `complete` o `abandon`; ignorar uno hasta su fecha límite ahora tiene consecuencias (consecuencias de la caducidad), y completar el favor de un compañero mueve la moral de ese compañero. Los arcos del poder creciente y del comerciante-príncipe al final del juego leen las oportunidades que realmente resolviste.
*   **Paridad de contenido en los diez mundos iniciales (v2.9):** cableado de equipos, misiones, compañeros reclutables y un saldo de monedas inicial implementados en cada mundo inicial que carecía de ellos; los diez mundos ahora comparten una superficie de características uniforme y completamente iluminada (el equipo era solo para gladiadores; las misiones eran solo de fantasía/zombis; cinco mundos se enviaron con `recruit` sin nadie a quien reclutar). Además, un validador de contenido estructural que detecta un ID de artículo escrito incorrectamente en cada superficie de referencia y ranuras de guardado con múltiples puntos de control con `--checkpoint`/`--list-checkpoints`.
*   **NPCs vivos, realmente vivos (v3.0):** el productor persistente de agencia NPC ilumina la sección **PERSONAS** del Director: los NPCs con nombre (un personaje de historia creado por mundo inicial, más todos los compañeros que reclutas) tienen metas, relaciones de confianza/miedo/avaricia/lealtad y una cadena de consecuencias. `runNpcAgencyTick` se ejecuta cada ronda, protegido para que un mundo sin NPCs con nombre siga siendo byte idéntico a la repetición heredada. Iluminar el productor también iluminó los puntos de interrupción de salida del favor del compañero y dos reglas de generación de oportunidades inactivas (meta NPC + obligación).
*   **La superficie social completa (v3.0):** los cuatro verbos de influencia se convierten en veinticinco: los grupos de diplomacia y sabotaje se registran (21 subverbos más), iluminando las reacciones de compañeros previamente oscuras `leverage-diplomacy` / `leverage-sabotage`; diecinueve aparecen en el menú numerado (afordabilidad + enfriamiento + reputación limitada). Las condiciones y los efectos del diálogo ahora leen y escriben el estado social (influencia / reputación / relación NPC). Y la influencia pasiva (`tickLeverage` / `computeLeverageGains`) gotea influencia de la reputación y otorga favor / chantaje / legitimidad de la experiencia y los hitos, por lo que la capa social se gana *entre* oportunidades, no solo al completarlas.
*   **Economía con sabor genérico (v3.0):** el inventario del comerciante y las recetas de artesanía ahora resuelven las tablas de género por mundo inicial (siete de diez mundos iniciales tienen contenido de género creado; tres retroceden a universales); en la mecánica de compra/artesanía, la pantalla del menú numerado y la sección RECETAS del Director, todo conectado desde la misma clave de conjunto de reglas para que la visualización y la mecánica coincidan. `repair` y `modify` son ahora filas del menú numerado (emparejamiento artículo × receta), y las oportunidades de `escort` surgen en una puerta de viaje protectora en un distrito peligroso.
*   **El final del juego lee la influencia que ganaste (v3.0):** los finales de campaña `victory`, `puppet-master` y `quiet-retirement`: antes limitados a la influencia / chantaje / legitimidad que la capa del final del juego leía como cero codificado, ahora son alcanzables a través del almacenamiento real de influencia que escribe toda la economía social. La salida del compañero también es alcanzable, a través de los puntos de interrupción de la agencia NPC y un respaldo de piso de moral.
*   **CLI de desarrollo `audit-content` (v3.0):** un comando de auditoría de contenido para desarrolladores (hermano de `validate`, distinto del Ledger del Director al que accede el jugador) que ejecuta los seis formateadores del director de encuentros / jefes / combate sobre un paquete.
*   **Suministro inicial con sabor genérico: el lanzamiento de v3.0, entregado (v3.1):** `economyGenre` conecta la clave del conjunto de reglas de cada mundo inicial a través de `buildWorldStack` → `createEconomyCore`, por lo que un distrito ahora genera su perfil `GENRE_SUPPLY_DEFAULTS` genérico (el cyberpunk tiene muchos componentes / contrabando, la fantasía tiene pocos medicamentos) en lugar de una línea de base universal plana: el suministro inicial que la visión general del mercado del Director y las entradas del final del juego ya leen. Siete de diez mundos iniciales tienen un perfil de género; tres retroceden a la línea de base, honestamente. Un campo separado de `tradeGenre` / `craftingGenre` para que los tres puedan divergir más adelante.
*   **La superficie social, completa (v3.1):** `deny` y `bury-scandal`: el par de manipulación de rumores que se dirige a un rumor existente por ID en lugar de una facción; alcanzan el menú numerado a través de una dimensión de emparejamiento objetivo de rumor, cerrando la superficie de veintiún verbos (19 → 21 mostrados).
*   **Diálogo `obligation-exists`, conectado y alcanzable (v3.1):** la condición del diálogo lee el libro mayor de obligaciones persistente de un NPC con nombre (`getPersistedNpcObligations`); la fantasía, el hermano Aldric, una vez que te debe un favor a través del juego normal de agencia NPC, desbloquea una opción `call-in-favor`: una puerta real donde v3.0 dejó un tocón siempre verdadero (una sesión jugada auditada en la fase 9 demostró que era alcanzable en una ejecución real, no solo en pruebas unitarias).
...
*   **Liquidación opcional del libro mayor XRPL (v3.2):** un nuevo paquete opcional `@ai-rpg-engine/ledger-adapter` vincula la capa comercial propiedad del jugador: `coin` → una promesa de pago, consumibles → tokens fungibles, el delta neto de `buy`/`sell` de un punto de control → un **escrow de token XLS-85** liquidado en la **testnet XRPL**, completamente fuera del núcleo determinista. Nada en `core`/`modules` lo importa y una ejecución es byte idéntica con o sin él (probado en el bucle real del comerciante `createGame()` pirata). Solo testnet detrás de una protección imposible en código, con un archivo secreto ignorado por git, reintentos seguros de conservación, verificación de memo en la cadena y una alternativa no anclada; probado en vivo de extremo a extremo en la testnet (liquidar mediante escrow de token → `reconcile` contra los saldos y memos del libro mayor). El equipo NFT único llega en v3.3 (a continuación). Consulte [El adaptador del libro mayor XRPL](#el-adaptador-del-libro-mayor-xrpl-opcional).
*   **Equipo único como NFTs (v3.3):** el `@ai-rpg-engine/ledger-adapter` vincula el equipo único del paquete `equipment`: la "porción posterior" diferida de v3.2 a los NFTs XRPL: cada artículo único acuñado como un **NFToken XLS-20** (`tfMutable`, nunca quemable: verdadera propiedad del jugador) en un punto de control, el crecimiento de la reliquia que avanza los metadatos de un NFT mutable en su lugar mediante **XLS-46 `NFTokenModify`**, y una familia de reconciliación `()` que verifica la propiedad en el libro mayor. Una ruta de lectura distinta sobre el inventario del equipo, transportada junto con la capa fungible: el mismo cortafuegos de determinismo, byte idéntico con o sin él. Probado en la sesión jugada real `starter-gladiator`, en vivo en la testnet (acuna el `trident-and-net` equipado como un NFT, posee en el libro mayor, reconcilia, el mundo no se ve perturbado).
*   **Equipo que gana un nombre (v3.4):** el crecimiento de la reliquia ha existido desde v3.3 pero nunca se había activado durante el juego: nada poblaba una crónica de artículos en un juego en ejecución, por lo que la versión de reliquia de cada artículo era permanentemente `0`. El lado de escritura ahora se envía: un módulo **`item-chronicle-core` opcional** registra `acquired` / `used-in-kill` / `recognized` del juego real, y `starter-gladiator` lo conecta: gana tres peleas de arena y el tridente retiarius es el **Tridente y la red ensangrentados** para el resto de la ejecución, mostrado en el HUD y el libro mayor del Director. Esto también **cierra el bucle NFT**: un punto de control liquida ese crecimiento como una verdadera **XLS-46 `NFTokenModify`**, avanzando la URI en el libro mayor mientras se preserva el NFTokenID. Arreglado en el camino: `boss-kill` y `recognized` eran inalcanzables en cada paquete enviado (una simple comprobación de etiqueta `boss` contra contenido que etiqueta `role:boss`, y una protección de facción contra contenido donde ninguna entidad establece una), y este último había estado bloqueando silenciosamente todo el crecimiento de la armadura. Opcional por construcción: un paquete que no lo conecta es byte idéntico al motor que se envió antes de su existencia.
*   `ai-rpg-engine create-starter <name>`: crea un nuevo juego (independiente, se ejecuta fuera del monorepo); comandos `validate` + `scaffold` para contenido; carga paquetes desde JSON.
*   Plantilla inicial publicada en npm (`@ai-rpg-engine/starter-template`).
*   Conjunto de pruebas completo: **5754 pruebas** (determinista entre ejecuciones repetidas; los archivos de prueba se verifican por tipo en CI; el cumplimiento de la cobertura se aplica).
...
`@ai-rpg-engine/ledger-adapter` es un paquete **opcional** que vincula la capa de objetos intercambiables propiedad de los jugadores de un juego —el saldo de `coin` y el inventario de objetos consumibles— a la **XRPL testnet**, de modo que esos activos puedan estar respaldados por tokens reales en el libro mayor y liquidarse en puntos de control. La ausencia del adaptador es exactamente el motor offline que se distribuye actualmente.

**La invariante del determinismo (el objetivo principal).** El adaptador es un *canal secundario*, nunca parte de la simulación:

- Nunca se invoca dentro del ciclo determinista, solo en los **puntos de control** (guardado, entrada a la ciudad/mercado, final de capítulo).
- Nada en `@ai-rpg-engine/core` o `@ai-rpg-engine/modules` lo importa (su única dependencia con el motor es un `import type` en tiempo de compilación).
- **Una ejecución es idéntica en bytes, con o sin él.** Una prueba de firewall ejecuta el bucle del comerciante `starter-pirate` `createGame()` en dos motores: uno con el adaptador habilitado y liquidando en un punto de control, y luego verifica que los dos mundos sean profundamente iguales. La reproducción con la semilla 0 no se ve afectada.
...
**Tres modos de juego.** `offline` (predeterminado: sin cadena, el motor tal como se distribuye), `ledger` (monedas/objetos respaldados por saldos de la testnet, liquidados en puntos de control) y `diary` (jugar offline y luego anclar el hash del estado de la ejecución en el libro mayor para obtener un recibo a prueba de manipulaciones).

**Qué hay en el libro mayor.** `coin`: una promesa de pago en moneda emitida sobre una línea de confianza; objetos consumibles: tokens fungibles; el delta neto de intercambio de un punto de control: una transferencia liquidada mediante **XLS-85 token escrow**. El equipo único se distribuye como **NFTs XLS-20** (v3.3), y el crecimiento de las reliquias avanza los metadatos de un NFT mutable en su lugar a través de **XLS-46 `NFTokenModify`**, impulsado por el juego real a partir de la v3.4. La economía abstracta del distrito (`economy-core`) *no* se ve afectada; sigue siendo una simulación pura.
...
| [`@ai-rpg-engine/equipment`](packages/equipment) | Tipos de equipo, procedencia de los objetos y crecimiento de las reliquias, incluido `item-chronicle-core`, el módulo opcional que registra el historial del equipo a partir del juego real para que los objetos obtengan epítetos y niveles. |
...
El motor principal es una **biblioteca de simulación solo local**: sin telemetría, sin red, sin secretos. Los archivos de guardado se guardan en `.ai-rpg-engine/` solo cuando se solicita explícitamente. Dos capas **opcionales** agregan una ruta de salida y solo cuando las invocas:
...
- La capa del libro mayor (`@ai-rpg-engine/ledger-adapter`) se conecta a la **XRPL testnet** y solo a la testnet: una protección estructural **imposible en el código para mainnet** (no una marca de configuración) rechaza cualquier host que no sea de testnet durante la construcción. Las semillas de la billetera se almacenan en un archivo secundario de secretos ignorado por Git, nunca en un archivo de guardado, y el núcleo determinista nunca importa el adaptador.
