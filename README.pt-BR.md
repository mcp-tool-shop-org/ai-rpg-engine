...
*   Sistema de combate: 5 ações, 4 estados de combate, 4 estados de engajamento, interceptação de companheiros, progressão da derrota, táticas de IA.
...
*   **O mundo reage (v2.7):** as mortes acumulam calor e corroem a segurança do distrito; um ciclo de jogo mundial por rodada gera pressões ocultas que surgem como rumores ("Sussurros chegam até você..."), aumentam e expiram com consequências; as ~30 composições de encontros criadas são acionadas na entrada da zona em todos os 10 mundos iniciais — determinísticas por semente, distritos mais sangrentos geram mais, peças-chave do chefe protegidas.
*   **Um motivo para retornar (v2.7):** um ciclo de missão mínimo no esquema já lançado — as missões oferecem gatilhos, rastreiam objetivos de morte/alcance/progresso e pagam experiência e itens exatamente uma vez; quatro missões criadas, uma tela de **Diário**, momentos da missão na narrativa da rodada.
*   **Equipamentos afetam o combate (v2.7):** `equip`/`unequip` movem números reais através da camada de status que as fórmulas de combate já leem — nenhuma alteração no código de combate; o tridente e a rede do gladiador são conectados de ponta a ponta com um delta de chance de acerto fixo em testes.
*   **Ciclos de jogo com semente (v2.7):** cada nova sessão imprime sua semente com o comando exato de repetição; `--seed <n>` reproduz uma sessão byte por byte; combate, resistência, habilidades e rolagens de táticas consomem todos a semente do mundo — e os finais leem o jogo que você realmente jogou (calor ao vivo, pressões, acúmulos de facções, nível do jogador).
...
*   **O diretor lê todo o tabuleiro (v2.8):** uma nova seção EQUIPAMENTO no Ledger (atrás da dependência de proveniência de equipamentos cli→), um trailer final do RESUMO DO DIRETOR, as seções VISÃO GERAL DO MERCADO + GRUPO agora alimentadas por produtores ao vivo e estabilidade do distrito + tom econômico na seção DISTRITOS do final.
*   **A outra metade da economia (v2.9):** um verbo `buy` completa o ciclo — estoque de mercador oferecido por distrito em granularidade de categoria de suprimento (o nível de suprimento *é* o sinal de reabastecimento), com preços através do mesmo pipeline `computeItemValue` que `sell` mais uma diferença de compra/venda para que não haja risco de viagem de ida e volta. E a criação ganha vida: `createCraftingCore` registra `salvage`/`craft`/`repair`/`modify` nas tabelas de receita criadas, acendendo as seções MATERIAIS + RECEITAS do diretor que foram lançadas inativas.
...
*   **A camada social, conectada de ponta a ponta (v2.9):** quatro verbos de alavancagem — `bribe`, `intimidate`, `petition`, `seed` (rumor) — escrevem globais reais de reputação / alerta / calor que negociam preços e portões de facção já lidos, e `seed` acende todo o módulo de rumores do jogador + a seção RUMORES SOBRE VOCÊ do diretor. A *economia* de alavancagem que os financia também está conectada: completar uma oportunidade agora concede a alavancagem que sempre narrou, para que os verbos sejam genuinamente obtidos no jogo.
*   **Oportunidades, o ciclo de vida completo (v2.9):** um gerador por rodada oferece contratos/recompensas/favores pontuados em relação ao estado mundial ao vivo; você `accept`, então `complete` ou `abandon`; ignorar até o prazo agora tem consequências (consequências da expiração) e completar o favor de um companheiro move a moral desse companheiro. Os arcos do poder crescente e do mercador-príncipe no final do jogo leem as oportunidades que você realmente resolveu.
*   **Paridade de conteúdo em todos os dez mundos iniciais (v2.9):** fiação de equipamentos, missões, companheiros recrutáveis e um saldo inicial de moedas lançados para cada mundo inicial que não os tinha — os dez mundos agora compartilham uma superfície de recursos uniforme e totalmente iluminada (o equipamento era apenas de gladiador; as missões eram apenas de fantasia/zumbi; cinco mundos foram lançados com `recruit` sem ninguém para recrutar). Além disso, um validador de conteúdo estrutural que detecta um ID de item digitado incorretamente em todas as superfícies de referência e slots de salvamento multi-checkpoint com `--checkpoint`/`--list-checkpoints`.
*   **NPCs vivos, realmente vivos (v3.0):** o produtor de agência NPC persistente acende a seção **PESSOAS** do diretor — NPCs nomeados (um personagem de história criado por mundo inicial, mais todos os companheiros que você recruta) carregam objetivos, relacionamentos de confiança/medo/ganância/lealdade, um livro de obrigações e cadeias de consequências. `runNpcAgencyTick` é executado a cada rodada, restrito para que um mundo sem NPCs nomeados permaneça byte a byte idêntico à repetição herdada. Acender o produtor também acendeu os pontos de interrupção de partida do companheiro e as duas regras de geração de oportunidades inativas (objetivo NPC + obrigação) e os perfis/obrigações NPC no final do jogo — o fio foi testado verde, mas inerte no conteúdo lançado até que uma auditoria da fase 9 o detectasse, então a correção lança um NPC nomeado criado em cada mundo inicial.
*   **A superfície social completa (v3.0):** os quatro verbos de alavancagem tornam-se vinte e cinco — os grupos de diplomacia e sabotagem se registram (21 subverbos adicionais), acendendo as reações de companheiros anteriormente escuras `leverage-diplomacy` / `leverage-sabotage`; dezenove aparecem no menu numerado (preço + tempo de recarga + reputação restrita). As condições e os efeitos do diálogo agora leem e escrevem o estado social (alavancagem / reputação / relacionamento NPC). E a renda de alavancagem passiva (`tickLeverage` / `computeLeverageGains`) goteja influência da reputação e concede favor / chantagem / legitimidade da experiência e dos marcos — para que a camada social ganhe *entre* as oportunidades, não apenas na conclusão.
*   **Economia com sabor de gênero (v3.0):** o estoque do mercador e as receitas de criação agora resolvem tabelas de gênero por mundo inicial (sete de dez mundos iniciais carregam conteúdo de gênero criado; três voltam para o universal, honestamente) — em toda a mecânica de compra/criação, na exibição do menu numerado e na seção RECEITAS do diretor, tudo derivado da mesma chave de conjunto de regras para que a exibição e a mecânica concordem. `repair` e `modify` são agora linhas do menu numerado (pareamento item × receita) e as oportunidades `escort` surgem em um portão de viagem protetora em um distrito perigoso.
*   **O final do jogo lê a alavancagem que você ganhou (v3.0):** os finais da campanha `victory`, `puppet-master` e `quiet-retirement` — antes restritos à influência / chantagem / legitimidade que a camada final do jogo lia como zero codificado — agora são alcançáveis através do armazenamento de alavancagem real que toda a economia social escreve. A partida do companheiro também é alcançável, por meio de pontos de interrupção da agência NPC e um limite mínimo de moral.
*   **CLI de auditoria de conteúdo (v3.0):** um comando CLI de auditoria de conteúdo para desenvolvedores (irmão de `validate`, distinto do Ledger do diretor voltado para o jogador) que executa os seis formatadores de encontro / chefe / combate em um pacote.
*   `ai-rpg-engine create-starter <name>` — prepara um novo jogo (autônomo, executado fora do monorepo); comandos `validate` + `scaffold` de conteúdo; carrega pacotes de JSON.
