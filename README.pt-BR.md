...
### Próximo (o ciclo de ajuste e aprofundamento)

A v3.10 entregou a superfície para o usuário; o que foi registrado no processo define o próximo ciclo:

- **Reajuste da entrada de `bounty` (o personagem P1 com nome)** — a limpeza do ouvinte inicial deslocou o fluxo de eventos determinístico e a janela de bônus de quatro condições do black-flag-requiem não se manifesta mais em uma sessão natural; o mecanismo passa sinteticamente, e `faction-summons` (toda a sua cadeia de consequências) espera atrás dele — uma passagem de balanceamento dedicada com uma única alavanca, com o sistema de alcance como barreira.
- **Um fim honesto para uma luta que ninguém venceu** — fugir de uma luta que esvazia a zona não aciona nenhum evento hoje (regra: recuar não é vitória); um design de encontro com `outcome` que indica o fim (recuo/retirada) com sua própria característica e semântica de memória.
- **Companheiros se lembram de onde vieram** — o recrutamento atualmente reescreve um companheiro na facção compartilhada do grupo, então “a guilda de onde eles vieram ouve você” fica inatingível; preservar uma facção de origem em `CompanionState` desbloqueia o bônus de alavancagem da rota da facção, conforme o planejado.
- **Cognição de facção sem a etapa de registro** — a propagação de rumores e o enquadramento do observador ainda dependem fortemente de um registro explícito `createFactionCognition` que nenhum pacote lançado executa; derivar a filiação de `entity.faction` criado (e rotear a hostilidade do observador por meio de `affiliationOf` em vez da desigualdade bruta da facção) completa o que a recuperação de identidade da v3.10 começou.
- **Variedade de música de zona e extensibilidade do pacote** — a ponte de tom é determinística em primeiro lugar (mesma zona, mesmo tema); um lançamento determinístico por zona e uma superfície de tom→humor que pode ser criada no pacote são os próximos passos planejados, juntamente com a camada ambiente próspera ausente.
- **Lotes de construção em andamento que sobrevivem a uma falha** — a v3.10 alerta na saída; serializar `BuildState`/`TuningState` (incluindo gravações em etapas) no arquivo da sessão é a solução completa.
