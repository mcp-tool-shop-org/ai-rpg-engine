...
// the pack's real 'wits' stat (the precision half of zombieMinimalRuleset's
// combat/ability statMapping) would never influence AI perception. setup.ts
// must pass `{ perceptionStat: 'wits' }` explicitly.
...
describe('zombie setup — perception filter reads the real stat (F-bed426aa)', () => {
  it('detects the player when wits is high, even though instinct is low', () => {
    const engine = createGame(42);
    const zoneId = engine.store.state.locationId;
    engine.store.state.zones[zoneId].light = 0;
    engine.store.state.entities['survivor'].visibility = { hidden: true };

    // wits=20 (always clears threshold), instinct=1 (never would).
    // Detected only if the filter reads 'wits', not the 'instinct' fallback.
    engine.store.addEntity(makeObserver('perception-test-sharp', { wits: 20, instinct: 1 }, zoneId));

    engine.store.emitEvent('world.zone.entered', { zoneId }, { actorId: 'survivor' });

    const cog = getCognition(engine.world, 'perception-test-sharp');
    expect(believes(cog, 'survivor', 'present', true)).toBe(true);
  });

  it('fails to detect the player when wits is low, even though instinct is high', () => {
    const engine = createGame(42);
    const zoneId = engine.store.state.locationId;
    engine.store.state.zones[zoneId].light = 0;
    engine.store.state.entities['survivor'].visibility = { hidden: true };

    // wits=1 (never clears threshold), instinct=20 (always would).
    // Undetected only if the filter reads 'wits', not the 'instinct' fallback.
    engine.store.addEntity(makeObserver('perception-test-dull', { wits: 1, instinct: 20 }, zoneId));

    engine.store.emitEvent('world.zone.entered', { zoneId }, { actorId: 'survivor' });

    const cog = getCognition(engine.world, 'perception-test-dull');
    expect(believes(cog, 'survivor', 'present', true)).toBe(false);
  });
});
