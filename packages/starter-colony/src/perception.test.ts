...
// the pack's real 'awareness' stat (the precision half of colonyMinimalRuleset's
// combat/ability statMapping) would never influence AI perception. setup.ts
// must pass `{ perceptionStat: 'awareness' }` explicitly.
...
describe('colony setup — perception filter reads the real stat (F-bed426aa)', () => {
  it('detects the player when awareness is high, even though instinct is low', () => {
    const engine = createGame(42);
    const zoneId = engine.store.state.locationId;
    engine.store.state.zones[zoneId].light = 0;
    engine.store.state.entities['commander'].visibility = { hidden: true };

    // awareness=20 (always clears threshold), instinct=1 (never would).
    // Detected only if the filter reads 'awareness', not the 'instinct' fallback.
    engine.store.addEntity(makeObserver('perception-test-sharp', { awareness: 20, instinct: 1 }, zoneId));

    engine.store.emitEvent('world.zone.entered', { zoneId }, { actorId: 'commander' });

    const cog = getCognition(engine.world, 'perception-test-sharp');
    expect(believes(cog, 'commander', 'present', true)).toBe(true);
  });

  it('fails to detect the player when awareness is low, even though instinct is high', () => {
    const engine = createGame(42);
    const zoneId = engine.store.state.locationId;
    engine.store.state.zones[zoneId].light = 0;
    engine.store.state.entities['commander'].visibility = { hidden: true };

    // awareness=1 (never clears threshold), instinct=20 (always would).
    // Undetected only if the filter reads 'awareness', not the 'instinct' fallback.
    engine.store.addEntity(makeObserver('perception-test-dull', { awareness: 1, instinct: 20 }, zoneId));

    engine.store.emitEvent('world.zone.entered', { zoneId }, { actorId: 'commander' });

    const cog = getCognition(engine.world, 'perception-test-dull');
    expect(believes(cog, 'commander', 'present', true)).toBe(false);
  });
});
