...
//                            mirroring starter-gladiator's own T0 block)
...
import { buildCatalog, player, colonyAbilities, commanderTree, scientistDialogue, xpAwards, itemCatalog } from './content.js';

const dialogues = [scientistDialogue];
...
function insertCreatedCharacter(archetypeId: string) {
  ...
  const entity = resolveEntity(buildFor(archetypeId), buildCatalog, colonyMinimalRuleset);
...
const EXPECTED_OPEN: Record<string, number> = { 'field-engineer': 4, 'colony-commander': 4, 'outrider': 4 };
...
describe('T0-tag-gate: created characters see the pack ability kit', () => {
  for (const archetype of buildCatalog.archetypes) {
    it(`${archetype.id} passes the tag gates and gets a non-empty ability menu`, () => {
      ...
      const open = colonyAbilities.filter((a) => gateOpen(a, entity.tags));
      expect(open.length).toBe(EXPECTED_OPEN[archetype.id]);
      const available = getAvailableAbilities(engine.store.state, playerId, [...colonyAbilities]);
...
describe('T0-progression-ceiling: earnable XP covers the tree (computed over content)', () => {
  it('kills + dialogue + first-visits + boss bonus >= total tree cost', () => {
    ...
    const treeCost = commanderTree.nodes.reduce((sum, n) => sum + n.cost, 0);
    const earnable =
...
describe('T0-verb-honesty-content: help rows match registered handlers', () => {
  it('every advertised verb resolves to a registered handler', () => {
    ...
    for (const verb of colonyMinimalRuleset.verbs) {
  ...
  it('brace and reposition (registered by the combat stack) appear in help', () => {
    const helped = new Set(colonyMinimalRuleset.verbs.map((v) => v.id));
...
describe('T0-equipment-truth: every entry path can reach the equip loop (computed)', () => {
  ...
  // STARTINGINVENTORY GUARD (inline, self-contained — no shared helper): a
  // typo'd item id in any archetype's or background's startingInventory must
  // fail loudly here rather than silently ship an unequippable kit.
