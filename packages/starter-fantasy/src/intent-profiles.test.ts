...
//
// The shipped game's enemies never acted in part because this starter passed a
...
import { createGame, fantasyIntentProfiles } from './setup.js';
...
describe('starter-fantasy — intent profile wiring (F1-cs-a)', () => {
  it('provides a non-empty list of built-in intent profiles to cognition', () => {
    expect(fantasyIntentProfiles.length).toBeGreaterThan(0);
    for (const profile of fantasyIntentProfiles) {
  ...
  it('every hostile entity declares a profileId that resolves to a provided profile', () => {
    ...
    const provided = new Set(fantasyIntentProfiles.map((p) => p.id));
  ...
  it('resolved profiles produce intents — hostiles can act on an intruder', () => {
    ...
    for (const hostile of hostilesOf(world)) {
      const profile = fantasyIntentProfiles.find((p) => p.id === hostile.ai?.profileId);
