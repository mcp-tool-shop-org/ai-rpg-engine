...
// 'healing-draught' reward missing from itemCatalog) ship silently — no
// suite anywhere checked that a quest's item rewards actually resolve.
//
// This file closes the gap for chapel-threshold: every quest, the dialogue,
// the ability pack, and the status pack now run through content-schema on
// every test run.
...
describe('schema-conformance: The Chapel Threshold content validates against content-schema', () => {
  for (const quest of fantasyQuests) {
  ...
  it('dialogue is structurally valid', () => {
    const result = validateDialogueDefinition(pilgrimDialogue);
    expect(result.ok, formatErrors(result)).toBe(true);
  });
