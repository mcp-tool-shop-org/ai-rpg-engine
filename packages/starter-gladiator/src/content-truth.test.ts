...
import { buildCatalog, player, gladiatorAbilities, arenaGloryTree, patronDialogue, xpAwards, itemCatalog } from './content.js';

const dialogues = [patronDialogue];
...
function insertCreatedCharacter(archetypeId: string) {
  ...
  const entity = resolveEntity(buildFor(archetypeId), buildCatalog, gladiatorMinimalRuleset);
...
const EXPECTED_OPEN: Record<string, number> = { 'retiarius': 4, 'murmillo': 4, 'provocator': 4 };
...
describe('T0-tag-gate: created characters see the pack ability kit', () => {
  for (const archetype of buildCatalog.archetypes) {
    it(`${archetype.id} passes the tag gates and gets a non-empty ability menu`, () => {
      ...
      const open = gladiatorAbilities.filter((a) => gateOpen(a, entity.tags));
      expect(open.length).toBe(EXPECTED_OPEN[archetype.id]);
      const available = getAvailableAbilities(engine.store.state, playerId, [...gladiatorAbilities]);
...
describe('T0-progression-ceiling: earnable XP covers the tree (computed over content)', () => {
  it('kills + dialogue + first-visits + boss bonus >= total tree cost', () => {
    ...
    const treeCost = arenaGloryTree.nodes.reduce((sum, n) => sum + n.cost, 0);
    const earnable =
...
describe('T0-verb-honesty-content: help rows match registered handlers', () => {
  it('every advertised verb resolves to a registered handler', () => {
    ...
    for (const verb of gladiatorMinimalRuleset.verbs) {
  ...
  it('equip/unequip help rows resolve to registered handlers (F-ENG008 wiring)', () => {
    const registered = new Set(createGame(11).getAvailableActions());
    const helped = new Set(gladiatorMinimalRuleset.verbs.map((v) => v.id));
