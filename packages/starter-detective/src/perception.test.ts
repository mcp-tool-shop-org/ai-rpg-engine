...
// the pack's headline 'perception' stat (the precision half of
// detectiveMinimalRuleset's combat/ability statMapping) would never
// influence AI perception. setup.ts must pass
// `{ perceptionStat: 'perception' }` explicitly.
...
describe('detective setup — perception filter reads the real stat (F-bed426aa)', () => {
  it('detects the player when perception is high, even though instinct is low', () => {
    const engine = createGame(42);
    const zoneId = engine.store.state.locationId;
    engine.store.state.zones[zoneId].light = 0;
    engine.store.state.entities['inspector'].visibility = { hidden: true };

    // perception=20 (always clears threshold), instinct=1 (never would).
    // Detected only if the filter reads 'perception', not the 'instinct' fallback.
    engine.store.addEntity(makeObserver('perception-test-sharp', { perception: 20, instinct: 1 }, zoneId));

    engine.store.emitEvent('world.zone.entered', { zoneId }, { actorId: 'inspector' });

    const cog = getCognition(engine.world, 'perception-test-sharp');
    expect(believes(cog, 'inspector', 'present', true)).toBe(true);
  });

  it('fails to detect the player when perception is low, even though instinct is high', () => {
    const engine = createGame(42);
    const zoneId = engine.store.state.locationId;
    engine.store.state.zones[zoneId].light = 0;
    engine.store.state.entities['inspector'].visibility = { hidden: true };

    // perception=1 (never clears threshold), instinct=20 (always would).
    // Undetected only if the filter reads 'perception', not the 'instinct' fallback.
    engine.store.addEntity(makeObserver('perception-test-dull', { perception: 1, instinct: 20 }, zoneId));

    engine.store.emitEvent('world.zone.entered', { zoneId }, { actorId: 'inspector' });

    const cog = getCognition(engine.world, 'perception-test-dull');
    expect(believes(cog, 'inspector', 'present', true)).toBe(false);
  });
});
