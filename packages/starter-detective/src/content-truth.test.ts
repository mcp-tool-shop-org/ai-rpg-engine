...
import { buildCatalog, player, detectiveAbilities, deductionTree, widowDialogue, xpAwards, itemCatalog } from './content.js';

const dialogues = [widowDialogue];
...
function insertCreatedCharacter(archetypeId: string) {
  ...
  const entity = resolveEntity(buildFor(archetypeId), buildCatalog, detectiveMinimalRuleset);
...
const EXPECTED_OPEN: Record<string, number> = { 'inspector': 4, 'raconteur': 4, 'bruiser': 4 };
...
describe('T0-tag-gate: created characters see the pack ability kit', () => {
  for (const archetype of buildCatalog.archetypes) {
    it(`${archetype.id} passes the tag gates and gets a non-empty ability menu`, () => {
      ...
      const open = detectiveAbilities.filter((a) => gateOpen(a, entity.tags));
      expect(open.length).toBe(EXPECTED_OPEN[archetype.id]);
      const available = getAvailableAbilities(engine.store.state, playerId, [...detectiveAbilities]);
...
describe('T0-progression-ceiling: earnable XP covers the tree (computed over content)', () => {
  it('kills + dialogue + first-visits + boss bonus >= total tree cost', () => {
    ...
    const treeCost = deductionTree.nodes.reduce((sum, n) => sum + n.cost, 0);
    const earnable =
...
describe('T0-verb-honesty-content: help rows match registered handlers', () => {
  it('every advertised verb resolves to a registered handler', () => {
    ...
    for (const verb of detectiveMinimalRuleset.verbs) {
  ...
  it('brace and reposition (registered by the combat stack) appear in help', () => {
    const helped = new Set(detectiveMinimalRuleset.verbs.map((v) => v.id));
