...
//
// This file closes the gap for gaslight-detective: every quest, the
// dialogue, the ability pack, and the status pack now run through
// content-schema on every test run. (F-c07d6024 added the two authored
// quests; the same class of bug F-a7a22999/F-b34a5c82 caught elsewhere — an
// item reward missing from itemCatalog — gets its own check below.)
...
import { widowDialogue, detectiveAbilities, detectiveStatusDefinitions, detectiveQuests, itemCatalog } from './content.js';

describe('schema-conformance: Gaslight Detective content validates against content-schema', () => {
  for (const quest of detectiveQuests) {
  ...
  it('dialogue is structurally valid', () => {
    const result = validateDialogueDefinition(widowDialogue);
    expect(result.ok, formatErrors(result)).toBe(true);
  });
