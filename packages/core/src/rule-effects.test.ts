...
describe('core-c1 — registered rule effects actually execute (meta-test)', () => {
  ...
  it('F-208d62e4: an effect.apply that returns [{ payload }] with no type degrades to rule.effect.failed naming the effect and index', () => {
    const mod: EngineModule = {
      ...
      register(ctx) {
        ...
        ctx.rules.registerEffect({
          id: 'fx-typeless',
          apply: () => [{ payload: { x: 1 } }] as unknown as ResolvedEvent[],
        });
      },
    };
    ...
    const failed = engine.world.eventLog.find((e) => e.type === 'rule.effect.failed');
    expect(failed).toBeDefined();
    expect(failed!.payload.effectId).toBe('fx-typeless');
