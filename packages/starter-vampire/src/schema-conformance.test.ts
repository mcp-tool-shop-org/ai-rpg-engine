...
//
// This file closes the gap for crimson-court: the dialogue, the ability
// pack, and the status pack now run through content-schema on every test
// run. (This pack defines no quests today — the quest check lives in
...
import { duchessDialogue, vampireAbilities, vampireStatusDefinitions } from './content.js';

describe('schema-conformance: Crimson Court content validates against content-schema', () => {
  it('dialogue is structurally valid', () => {
    const result = validateDialogueDefinition(duchessDialogue);
  ...
  it('status definitions are structurally valid with unique ids', () => {
    const result = validateStatusPackAgainstRuleset(vampireStatusDefinitions, vampireMinimalRuleset);
    expect(result.ok, formatErrors(result)).toBe(true);
  });
});
