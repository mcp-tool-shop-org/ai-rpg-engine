...
export function createGame(seed?: number): Engine {
  ...
  const worldStack = buildWorldStack({
    ...
    districts,
    presentationRules: [vampireHungerPerception],
    ...
    // genre key ('vampire-minimal' minus its '-minimal' suffix). 'vampire'
    // has no GENRE_BUYABLE_STOCK/GENRE_RECIPES entry (today's fixed key set
    // is fantasy/cyberpunk/pirate/zombie/detective/colony/weird-west), so
    // buy/craft/repair/modify correctly keep resolving the universal/default
