...
import { buildCatalog, player, vampireAbilities, bloodMasteryTree, duchessDialogue, xpAwards, itemCatalog } from './content.js';

const dialogues = [duchessDialogue];
...
function insertCreatedCharacter(archetypeId: string) {
  ...
  const entity = resolveEntity(buildFor(archetypeId), buildCatalog, vampireMinimalRuleset);
...
const EXPECTED_OPEN: Record<string, number> = { 'courtier': 4, 'stalker': 4, 'aesthete': 4 };
...
describe('T0-tag-gate: created characters see the pack ability kit', () => {
  for (const archetype of buildCatalog.archetypes) {
    it(`${archetype.id} passes the tag gates and gets a non-empty ability menu`, () => {
      ...
      const open = vampireAbilities.filter((a) => gateOpen(a, entity.tags));
      expect(open.length).toBe(EXPECTED_OPEN[archetype.id]);
      const available = getAvailableAbilities(engine.store.state, playerId, [...vampireAbilities]);
...
describe('T0-progression-ceiling: earnable XP covers the tree (computed over content)', () => {
  it('kills + dialogue + first-visits + boss bonus >= total tree cost', () => {
    ...
    const treeCost = bloodMasteryTree.nodes.reduce((sum, n) => sum + n.cost, 0);
    const earnable =
...
describe('T0-verb-honesty-content: help rows match registered handlers', () => {
  it('every advertised verb resolves to a registered handler', () => {
    ...
    for (const verb of vampireMinimalRuleset.verbs) {
  ...
  it('brace and reposition (registered by the combat stack) appear in help', () => {
    const helped = new Set(vampireMinimalRuleset.verbs.map((v) => v.id));
