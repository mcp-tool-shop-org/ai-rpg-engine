...
**構成例**：このサンプルは、ゲームエンジンを「封建ミステリー」のテーマに合うように設定する方法を示しています。これは参考として活用し、そのままコピーするテンプレートではありません。独自のゲームをビルドするには、[構成ガイド](../../docs/handbook/57-composition-guide.md)を参照してください。
...
| 特徴 | Roninが表現するもの |
|---------|------------------|
| **Engagement** | 複数の保護者役割（ボディガード＋侍）、隠された通路 |
| **Resources** | 二層構造：気（回復可能） vs 名誉（脆く、回復が困難） |
| **Social** | 結果を伴う調査：誤った告発は名誉を失わせる |
| **Cognition** | 暗殺者の感知ルール：特定の条件を満たす、関係のない浪人に対して適用 |
...
複数の保護者役割（ボディガード＋侍）と、二層構造のリソース（気＋名誉）。異なるトリガー条件を持つ2つの保護者役割が、どのように多層的な防御を構築するか、そして、気（回復可能）と名誉（脆く、回復が困難）が、戦闘と調査において異なるプレイスタイルを促すかを研究してください。
