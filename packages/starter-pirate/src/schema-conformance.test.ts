...
// resolve.
//
// This file closes the gap for black-flag-requiem: every quest (F-c07d6024),
...
import { cartographerDialogue, pirateAbilities, pirateStatusDefinitions, pirateQuests, itemCatalog } from './content.js';

describe('schema-conformance: Black Flag Requiem content validates against content-schema', () => {
  for (const quest of pirateQuests) {
  ...
  it('dialogue is structurally valid', () => {
    const result = validateDialogueDefinition(cartographerDialogue);
    expect(result.ok, formatErrors(result)).toBe(true);
  });
