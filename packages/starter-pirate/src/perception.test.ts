...
// the pack's real 'cunning' stat (the precision half of pirateMinimalRuleset's
...
describe('pirate setup — perception filter reads the real stat (F-bed426aa)', () => {
  it('detects the player when cunning is high, even though instinct is low', () => {
    ...
    engine.store.state.entities['captain'].visibility = { hidden: true };
    ...
    engine.store.emitEvent('world.zone.entered', { zoneId }, { actorId: 'captain' });

    const cog = getCognition(engine.world, 'perception-test-sharp');
    expect(believes(cog, 'captain', 'present', true)).toBe(true);
  });
  ...
  it('fails to detect the player when cunning is low, even though instinct is high', () => {
    ...
    expect(believes(cog, 'captain', 'present', true)).toBe(false);
  });
});
