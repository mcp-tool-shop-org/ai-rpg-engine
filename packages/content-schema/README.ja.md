...
- **部屋のスキーマ:** 出口、属性、および環境状態を持つエリア
- **エンティティのスキーマ:** NPC（ノンプレイヤーキャラクター）、クリーチャー、およびプレイヤーキャラクターの定義
- **会話のスキーマ:** 条件と効果を持つ、グラフベースの会話ツリー
- **アイテムのスキーマ:** 装備品、消耗品、クエストアイテム、およびステータス修正値
- **コンテンツパックローダー:** JSON/TypeScript形式のコンテンツパックを検証し、読み込む機能
- **アビリティのスキーマ:** アビリティの定義、ステータスの定義、およびバランスに関する注意点を含むパックの検証
- **スキーマバリデーター:** 実行時検証機能。構造化されたエラーメッセージを表示します。
