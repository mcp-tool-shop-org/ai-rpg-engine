...
`generateSweepValues(from, to, step)` produces float-safe ranges, capped at `MAX_EXPERIMENT_RUNS` (a tiny step is truncated, same bound as `seedList`). `runParameterSweep` slices `sweepSpec.values` so `values.length * runsPerPoint` stays at `MAX_EXPERIMENT_RUNS` (a 10000-value axis with `runs>1` cannot execute 10000 full batches). Each point runs the full experiment and a recommendation is generated from results.
