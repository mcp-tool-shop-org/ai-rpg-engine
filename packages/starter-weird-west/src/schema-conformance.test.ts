...
// ollama's AI-draft content. That gap let F-a7a22999 (wardensRestQuest's
// 'healing-draught' reward missing from itemCatalog, in starter-fantasy) ship
// silently — no suite anywhere checked that a quest's item rewards actually
// resolve.
//
// This file closes the gap for dust-devils-bargain: every quest (F-c07d6024),
...
import { bartenderDialogue, weirdWestAbilities, weirdWestStatusDefinitions, weirdWestQuests, itemCatalog } from './content.js';

describe("schema-conformance: Dust Devil's Bargain content validates against content-schema", () => {
  for (const quest of weirdWestQuests) {
  ...
  it('dialogue is structurally valid', () => {
    const result = validateDialogueDefinition(bartenderDialogue);
    expect(result.ok, formatErrors(result)).toBe(true);
  });
