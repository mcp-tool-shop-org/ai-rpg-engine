...
//                            grants (F-ENG008/F-86b9145d: the equip loop must
//                            be reachable from every way a player enters play)
...
import { buildCatalog, player, weirdWestAbilities, gunslingerTree, bartenderDialogue, xpAwards, itemCatalog } from './content.js';

const dialogues = [bartenderDialogue];
...
function insertCreatedCharacter(archetypeId: string) {
  ...
  const entity = resolveEntity(buildFor(archetypeId), buildCatalog, weirdWestMinimalRuleset);
...
const EXPECTED_OPEN: Record<string, number> = { 'gunslinger': 2, 'spirit-walker': 3, 'lawkeeper': 2 };
...
describe('T0-tag-gate: created characters see the pack ability kit', () => {
  for (const archetype of buildCatalog.archetypes) {
    it(`${archetype.id} passes the tag gates and gets a non-empty ability menu`, () => {
      ...
      const open = weirdWestAbilities.filter((a) => gateOpen(a, entity.tags));
      expect(open.length).toBe(EXPECTED_OPEN[archetype.id]);
      const available = getAvailableAbilities(engine.store.state, playerId, [...weirdWestAbilities]);
...
describe('T0-progression-ceiling: earnable XP covers the tree (computed over content)', () => {
  it('kills + dialogue + first-visits + boss bonus >= total tree cost', () => {
    ...
    const treeCost = gunslingerTree.nodes.reduce((sum, n) => sum + n.cost, 0);
    const earnable =
...
describe('T0-verb-honesty-content: help rows match registered handlers', () => {
  it('every advertised verb resolves to a registered handler', () => {
    ...
    for (const verb of weirdWestMinimalRuleset.verbs) {
  ...
  it('brace and reposition (registered by the combat stack) appear in help', () => {
    const helped = new Set(weirdWestMinimalRuleset.verbs.map((v) => v.id));
...
describe('T0-unreachable-abilities: dust-devil has real grant paths', () => {
  it('the spirit-walker archetype carries the supernatural gate natively', () => {
    const { entity } = insertCreatedCharacter('spirit-walker');
    expect(entity.tags).toContain('supernatural');
    expect(gateOpen(weirdWestAbilities.find((a) => a.id === 'dust-devil')!, entity.tags)).toBe(true);
  });

  it('the Spirit-Touched trait grants the supernatural gate to any archetype', () => {
    const build = buildFor('gunslinger');
    build.traitIds = [...build.traitIds, 'spirit-touched'];
    const entity = resolveEntity(build, buildCatalog, weirdWestMinimalRuleset);
    expect(entity.tags).toContain('supernatural');
    expect(gateOpen(weirdWestAbilities.find((a) => a.id === 'dust-devil')!, entity.tags)).toBe(true);
  });
});
