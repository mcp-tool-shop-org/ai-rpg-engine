...
import { createGame, weirdWestIntentProfiles } from './setup.js';
...
describe('starter-weird-west — intent profile wiring (F1-cs-a)', () => {
  it('provides a non-empty list of built-in intent profiles to cognition', () => {
    expect(weirdWestIntentProfiles.length).toBeGreaterThan(0);
    for (const profile of weirdWestIntentProfiles) {
  ...
  it('every hostile entity declares a profileId that resolves to a provided profile', () => {
    ...
    const provided = new Set(weirdWestIntentProfiles.map((p) => p.id));
  ...
  it('resolved profiles produce intents — hostiles can act on an intruder', () => {
    ...
    for (const hostile of hostilesOf(world)) {
      const profile = weirdWestIntentProfiles.find((p) => p.id === hostile.ai?.profileId);
