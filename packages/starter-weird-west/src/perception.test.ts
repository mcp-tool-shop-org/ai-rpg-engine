...
// the pack's real 'draw-speed' stat (the precision half of
// weirdWestMinimalRuleset's combat/ability statMapping) would never
// influence AI perception. setup.ts must pass
// `{ perceptionStat: 'draw-speed' }` explicitly.
...
describe('weird-west setup — perception filter reads the real stat (F-bed426aa)', () => {
  it('detects the player when draw-speed is high, even though instinct is low', () => {
    const engine = createGame(42);
    const zoneId = engine.store.state.locationId;
    engine.store.state.zones[zoneId].light = 0;
    engine.store.state.entities['drifter'].visibility = { hidden: true };

    // draw-speed=20 (always clears threshold), instinct=1 (never would).
    // Detected only if the filter reads 'draw-speed', not the 'instinct' fallback.
    engine.store.addEntity(makeObserver('perception-test-sharp', { 'draw-speed': 20, instinct: 1 }, zoneId));

    engine.store.emitEvent('world.zone.entered', { zoneId }, { actorId: 'drifter' });

    const cog = getCognition(engine.world, 'perception-test-sharp');
    expect(believes(cog, 'drifter', 'present', true)).toBe(true);
  });

  it('fails to detect the player when draw-speed is low, even though instinct is high', () => {
    const engine = createGame(42);
    const zoneId = engine.store.state.locationId;
    engine.store.state.zones[zoneId].light = 0;
    engine.store.state.entities['drifter'].visibility = { hidden: true };

    // draw-speed=1 (never clears threshold), instinct=20 (always would).
    // Undetected only if the filter reads 'draw-speed', not the 'instinct' fallback.
    engine.store.addEntity(makeObserver('perception-test-dull', { 'draw-speed': 1, instinct: 20 }, zoneId));

    engine.store.emitEvent('world.zone.entered', { zoneId }, { actorId: 'drifter' });

    const cog = getCognition(engine.world, 'perception-test-dull');
    expect(believes(cog, 'drifter', 'present', true)).toBe(false);
  });
});
