...
### Next (the tuning-and-depth cycle)

v3.10 delivered the consumer surface; what it recorded on the way out sets the next cycle:

- **Retune `bounty`'s on-ramp (the named P1)** — the starter-listener cleanup shifted the deterministic event stream and black-flag-requiem's four-condition bounty window no longer lands in a natural session; the mechanism passes synthetically, and `faction-summons` (its whole consequence chain) waits behind it — a dedicated single-lever balance pass with the reachability harness as the gate
- **An honest end for a fight nobody won** — fleeing a fight that then empties the zone fires no event today (ruled: retreat is not victory); an `outcome`-bearing encounter-ended design (retreat/withdrawn) with its own sting and memory semantics
- **Companions remember where they came from** — recruit currently rewrites a companion onto the party's shared faction, so "the guild they came from listens to you" is unreachable; preserving an origin faction on `CompanionState` unlocks the faction-route leverage bonus as designed
- **Faction cognition without the registration step** — rumor propagation and observer framing still hard-depend on an explicit `createFactionCognition` registration no shipped pack performs; deriving membership from authored `entity.faction` (and routing observer hostility through `affiliationOf` instead of raw faction inequality) finishes what v3.10's identity fallback started
- **Zone-music variety and pack extensibility** — the tone bridge is deterministic-first (same zone, same stem); a per-zone deterministic roll and a pack-authorable tone→mood surface are the designed next steps, alongside the missing prosperous-family ambient bed
- **In-flight build batches that survive a crash** — v3.10 warns on exit; serializing `BuildState`/`TuningState` (staged writes included) into the session file is the full fix
